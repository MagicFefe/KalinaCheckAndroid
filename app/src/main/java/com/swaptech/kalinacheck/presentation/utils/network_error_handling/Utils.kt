package com.swaptech.kalinacheck.presentation.utils.network_error_handling
...
fun <T: Any> handleRequest(block: () -> Response<T>) =
    try {
        ...
        if (response.isSuccessful && body != null) {
            ...
            Fail(
                code = response.code(),
                message = response.errorBody()?.string() ?: "NULL"
            )
        }
    } catch (httpException: HttpException) {
        Fail(
            code = httpException.code(),
            message = httpException.response()?.errorBody()?.string() ?: "NULL"
        )
